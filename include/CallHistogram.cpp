...
{
 /*
 Метод для эквализации изображения 
 Input:
 frameHistNew - бокс с объектом
 Output:
 frameHist - изображение над которым была выполнена эквализация 
*/
cv::Mat EqualizeHistFrame(const cv::Mat &frameHistNew)
{
    cv::Mat frameHist;
...
*/
void ImageHist(const cv::Mat &frameHistNew, std::vector<cv::Mat>& arrHist, int idGlobal) 
{
...
/*
 Метод для подсчета совпадений среди накопившихся гистограмм и новой 
 Input:
